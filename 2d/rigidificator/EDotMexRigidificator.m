classdef EDotMexRigidificator < Rigidificator
    ...
    methods
        function obj = EDotMexRigidificator()
        ...
        function checkForElastification( obj, mesh2D, cache, frame, h, settings )
            ...
            elseif ~settings.RigidificationEnabled
                ...
                F = cache.ActiveB * (mesh2D.p + cache.oldp)/2;
            ...
            if isempty(mesh2D.RigidBodies) || ~settings.ElastificationEnabled
                ...
                Fbuffer = mesh2D.B * (mesh2D.p + h*vel+mesh2D.p)/2;
                FApprox = Fbuffer(rigidRows);
                Fbuffer = mesh2D.B * vel; 
                FdotApprox = Fbuffer(rigidRows); 
                
                EDotApproxNorms = mexEdotNorm2D( FApprox, FdotApprox )';
