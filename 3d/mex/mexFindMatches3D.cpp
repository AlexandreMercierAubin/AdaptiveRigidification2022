/*==========================================================
 * mexFindMatches3D.c - mex find matches for warmstarting
 *
 * Find the matches between new contacts and old contacts. 
 *
 * The calling syntax is:
 *
 * [ warmStartLambdas, isNewContact ] = mexFindMatches3D( oldIDs, oldLambdas, newIDs );
 *
 ...
 *
 * To compile type: mex -R2018a mexFindMatches3D.cpp
 *========================================================*/
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) {
    ...
    if ( nrhs != 3 ) {
        mexErrMsgIdAndTxt("ARP:mFindMatches:nrhs","Nine inputs required.");
    }
    if ( nlhs != 2 ) {
    ...
    plhs[0] = mxCreateDoubleMatrix( nNew*3, 1, mxREAL ); // will be initialized to zero
    ...
    for ( int i = 0; i < nNew; i++ ) {
        ...
        if ( entry == umap.end() ) {
            ...
            int j = entry->second;
            warmStartLambdas[ i*3 ] = oldLambdas[ j*3 ];
            warmStartLambdas[ i*3+1 ] = oldLambdas[ j*3+1 ];
            warmStartLambdas[ i*3+2 ] = oldLambdas[ j*3+2 ];
        }
    }
}