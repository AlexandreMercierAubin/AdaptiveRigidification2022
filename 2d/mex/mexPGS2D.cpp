/*==========================================================
 * mexPGS2D.cpp - mex projected gauss seidel
 *
 ...
 * JcT must be sparse
 * For N contacts, many vectors will have 2N entires
 * Size of T is M-by-2N
 */
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) {
    double *lambda;             /* 2nContacts x 1 (output, duplicated from input) */
    double *deltav;             /* nDOFs x 1  (output, duplicated from input) */
    double *b;                  /* 2nContacts x 1 */
    double *Dii;                /* 1 x 2nContacts, do we care row/col? */
    ...
    if ( nrhs != 9 ) {
        mexErrMsgIdAndTxt("ARP:mexPGS2D:nrhs","Nine inputs required.");
    }
    if ( nlhs != 2 ) {
        mexErrMsgIdAndTxt("ARP:mexPGS2D:nlhs","Two outputs required.");
    }
    /* make sure the first input argument is scalar */
    if( !mxIsDouble(prhs[0]) || mxIsComplex(prhs[0]) || mxGetNumberOfElements(prhs[0])!=1 ) {
        mexErrMsgIdAndTxt("ARP:mexPGS2D:notScalar","iterations must be a scalar.");
    }
    /* make sure the second input argument is type double */
    if( !mxIsDouble(prhs[1]) || mxIsComplex(prhs[1]) ) {
        mexErrMsgIdAndTxt("ARP:mexPGS2D:notDouble","lambda vector must be type double.");
    }
    /* make sure the compliance input argument is scalar */
    if( !mxIsDouble(prhs[8]) || mxIsComplex(prhs[8]) || mxGetNumberOfElements(prhs[8])!=1 ) {
        mexErrMsgIdAndTxt("ARP:mexPGS2D:notScalar","compliance must be a scalar.");
    }
...
//     return;
    ...
    for ( int iter = 0; iter < iterations; iter++ ) {
        ...
        for ( int contact_index = 0; contact_index < nContacts; contact_index++ ) {
            ...
            // recall double *T has size nDOFs x 2nContacts 
