function [comparisons, initialMeshes, meshes, integrators, rigidificators] = setupComparisons2D(meshes, integrators, rigidificators)
...
%    comparisons = max([numel(meshes), numel(integrators), numel(rigidificators), numel(contactFinders)]);
    ...
    for k = 1:numel(meshes)
        initialMeshes{k} = meshes{k}.clone();
    end
